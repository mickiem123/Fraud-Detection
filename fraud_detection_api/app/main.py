...
def startup_event():
    ...
    HISTORICAL_PATH = os.path.join(
        "fraud_detection_api/data", "historical_transactions.csv"
    )
    RAW_DATA_DIR = "data/raw_data"

    if not os.path.exists(HISTORICAL_PATH):
        print(f"{HISTORICAL_PATH} not found. Creating from raw pickles...")
        dfs = []
        ...
        if dfs:
            ...
            full_df.to_csv(HISTORICAL_PATH, index=False)
            print(f"Created {HISTORICAL_PATH} with {len(full_df)} records.")
        else:
            print("No pickle files found in raw data directory. Cannot create historical data.")

    try:
        db_df = pd.read_csv(HISTORICAL_PATH)
        ...
        print(f"WARNING: {HISTORICAL_PATH} not found. Cache hydration will be skipped.")
